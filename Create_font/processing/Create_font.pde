...
/*
  ...
  This software is under construction, core processes are running.
*/

/*  >>>>>>>>>>>>>>>>>>>>   BE SURE TO MODIFY   myFontFiles variable   <<<<<<<<<<<<<<<<<<<< */

////////////////////////////////////////////////////////////////////////////////////////////////

// This is a processing sketch to create font files for TSDesktop.

// >>>>>>>>>>>>>>>>>>>>             INSTRUCTIONS             <<<<<<<<<<<<<<<<<<<<

// Created fonts are saved in the myFontFiles folder.
...
/*  >>>>>>>>>>>>>>>>>>>>      BE SURE TO MODIFY THE DIRECTORY BELOW      <<<<<<<<<<<<<<<<<<<< */

static final String myFontFiles = "/home/<your name>/FontFiles"; 
...
class Writer {
  ...
  // ----------------------------------------------------------------------------
  static public int pixels_font(final byte[] fontbuf, final int bpp, final int ver, int w, int h, int[] pixels)
  {
    int n = 0;
    byte u = 0;
    int i = 0;
...
class ToFile {
  ...
  public ToFile(final String fontName, final int bpp, final int ver, final int fontHeight)
  {
    this.bpp = bpp;
    this.ver = ver;
  ...
  int proc_char(Glyph glyph, short z, short w, short c) throws IOException
  {
    ...
    int w1 = (image.pixelWidth + 7) >> 3;
    byte[] fontbuf = new byte[image.pixelHeight * w1 * (1 << bpp)];
    int fontn = pixels_font(fontbuf, bpp, ver, image.pixelWidth, image.pixelHeight, image.pixels);
  ...
  //  GFXglyph t = new GFXglyph(); 
    ...
    int len0 = fontn % 0x100;
    int len1 = fontn / 0x100;
    writer.write("  0x%02X, 0x%02X, %d, %d, %d, %d, 0x%02x, 0x%02X, %d, %d,", code, bpp, ver, width, height, xAdvance, xOffset, yOffset, len0, len1);

    int j = 9;
    for (int i = 0; i < fontn; ++i) {
      writer.write(" 0x%02X,", fontbuf[i]);
      if (++j >= 15) {
        writer.write("\n   ");
        j = 0;
      }
  ...
  private void page_end()
  {
    ...
    int c1 = z > 0 ? z : w;
    int c2 = z > 0 ? w : 0;
    writer.write("  0x%02x, 0x%02x, 0, 0x%02X, 0x%02X, %d\n};\n\n", c1, c2, fr, to, fontHeight);
  }
  ...
  void run(final String fontPath, final String fontName, final String fontType, final char[] charset, final int fontSize, final int fontSize2, final int fontHeight)
  {
...
//    byte bpp = 0;  // 1 bit per pixel
//    byte bpp = 1;  // 2 bits per pixel
    byte bpp = 2;  // 4 bits per pixel
//    byte bpp = 3;  // 8 bits per pixel
//    byte ver = 0;  // bitmaps compressed
    byte ver = 1;  // bitmaps not compressed
    ToFile toFile = new ToFile(fontNameSizePt, bpp, ver, fontHeight); 
    try {
      String currentPath = new java.io.File(".").getCanonicalPath();
      System.out.println("Current dir:" + currentPath);
      
      toFile.run(font);
      File file = new File(myFontFiles + "/TSD_" + fontNameSizePt + ".h");
...
// fontNumber, fontPath, fontName, fontType, displayFontSize are used to display fonts only.
// Operating parameters should be given in the run function.
...
String fontPath = "/usr/share/fonts/truetype/freefont/";  // Manually crop the filename length later after creation if needed
String fontName = "FreeSans";  // Manually crop the filename length later after creation if needed
...
void setup() {
  ...
  myFont = createFont(fontPath+fontName+fontType, displayFontSize, smooth, charset);
  ...
  for (int y = 0; y < height-gapy; y += gapy) {
    ...
    run("/usr/share/fonts/truetype/freefont/", "FreeMono", ".ttf", charset,  9, 20, 19);
    run("/usr/share/fonts/truetype/freefont/", "FreeMono", ".ttf", charset, 12, 24, 24);
    run("/usr/share/fonts/truetype/freefont/", "FreeMono", ".ttf", charset, 18, 36, 35);
    run("/usr/share/fonts/truetype/freefont/", "FreeMono", ".ttf", charset, 24, 48, 47);

    run("/usr/share/fonts/truetype/freefont/", "FreeSans", ".ttf", charset,  9, 20, 22);
    run("/usr/share/fonts/truetype/freefont/", "FreeSans", ".ttf", charset, 12, 24, 29);
    run("/usr/share/fonts/truetype/freefont/", "FreeSans", ".ttf", charset, 18, 36, 44);  // 42
    run("/usr/share/fonts/truetype/freefont/", "FreeSans", ".ttf", charset, 24, 48, 58);  // 56

    run("/usr/share/fonts/truetype/freefont/", "FreeSerif", ".ttf", charset,  9, 20, 22);
    run("/usr/share/fonts/truetype/freefont/", "FreeSerif", ".ttf", charset, 12, 24, 29);
    run("/usr/share/fonts/truetype/freefont/", "FreeSerif", ".ttf", charset, 18, 36, 44);  // 42
    run("/usr/share/fonts/truetype/freefont/", "FreeSerif", ".ttf", charset, 24, 48, 58);  // 56
...
/*
  ...
  font = createFont(fontPath+fontName+fontType, fontSize, smooth, charset); //<>//
